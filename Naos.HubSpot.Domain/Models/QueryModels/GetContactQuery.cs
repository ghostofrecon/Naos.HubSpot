...
namespace Naos.HubSpot.Domain.Models.QueryModels
{
    ...
    public class GetContactQuery
    {
        ...
        /// <summary>
        /// Overrides the default ToString() method to produce a query string.
        /// </summary>
        /// <returns>The query string needed to fulfill the http request.</returns>
        public override string ToString()
        {
            var paramList = new List<string>()
            {
                this.properties.Length < 1 ? null : $"property={string.Join("&property=", this.properties)}",
                string.IsNullOrWhiteSpace(this.propertyMode) ? null : $"propertyMode={this.propertyMode}",
                string.IsNullOrWhiteSpace(this.formSubmissionMode) ? null : $"formSubmissionMode={this.formSubmissionMode}",
                this.showListMemberships ? null : $"showListMemberships={this.showListMemberships}",
            };
